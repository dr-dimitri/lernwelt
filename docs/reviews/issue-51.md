...
- Die aufwendigere Grafik wird in der Pause nur einmal gezeichnet; die Animationsschleife ruht. Resize zeichnet neu, Fortsetzen startet die Schleife wieder. Der bestehende Pausentest sichert das zusätzliche Verhalten ab. Im abschließenden Review wurde außerdem ein HUD-Randfall der neuen Pausenoptimierung korrigiert: Ein unmittelbar vor Pause erzielter Treffer wird im einmaligen Pausenframe sofort übernommen. Ein eigener UI-Test sichert den Punktestand ab, bevor das reguläre 100-ms-HUD-Intervall abläuft. Animationen verwenden ausschließlich die aktive Spielzeit/Position, keine zusätzliche Uhr.
...
- `npm run check:all` erfolgreich: 71 Frontend-/Spieltests, 2 Workflowtests, 57 Rusttests (130 insgesamt), Prettier, TypeScript/Vite, rustfmt und Clippy. Nach der abschließenden HUD-Korrektur den vollständigen Check und nativen Release-Build erneut erfolgreich ausgeführt. Den Nachtrag separat auf Interaktion mit der pausierten Zeichenroutine und dem Rundenende geprüft.
