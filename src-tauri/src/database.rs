...
fn migrate(connection: &mut Connection) -> Result<(), String> {
    ...
    if version < 4 {
        transaction
            .execute_batch(include_str!("../migrations/004_arcade.sql"))
            .map_err(database_error)?;
    }
    if version < 5 {
        transaction
            .execute_batch(include_str!("../migrations/005_difficulty_points.sql"))
            .map_err(database_error)?;
    }
    if version < 6 {
        transaction
            .execute_batch(include_str!("../migrations/006_vocabulary.sql"))
            .map_err(database_error)?;
    }
    if version < 7 {
        transaction
            .execute_batch(include_str!("../migrations/007_vocabulary_points.sql"))
            .map_err(database_error)?;
    }
    if version < 8 {
        transaction
            .execute_batch(include_str!("../migrations/008_learning_points.sql"))
            .map_err(database_error)?;
    }
    if version < 9 {
        transaction
            .execute_batch(include_str!("../migrations/009_multiplication.sql"))
            .map_err(database_error)?;
    }
    if version < 10 {
        transaction
